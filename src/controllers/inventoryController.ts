...
function getEffectiveOutletId(req: AuthRequest): number | null {
  const raw =
    req.user?.role === "super_admin"
      ? (req.query as any).outletId
      : req.user?.outletId;
  if (!raw) return null;
  const n = Number(raw);
...
export const getInventory = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    res.json({
      inventory: result.rows.map((row: any) => ({
        id: row.id,
    ...
    if (isDbConnectionError(error)) {
      ...
      for (const p of mockProducts.filter((pp) => pp.is_active)) {
      ...
      let rows = mockInventory.filter((i) => i.outlet_id === outletId);
      let joined = rows
        .map((inv) => {
          const p = mockProducts.find((pp) => pp.id === inv.product_id);
          return {
            id: inv.id,
      ...
      const sliced = joined.slice(off, off + lim).map((row: any) => ({
        id: row.id,
        outletId: row.outlet_id,
        productId: row.product_id,
        variantId: row.variant_id,
...
export const adjustInventory = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const variantIdNorm =
      variantId === "" || variantId === undefined ? null : Number(variantId);
    if (
      variantIdNorm !== null &&
    ...
    try {
      ...
      try {
        ...
        let inventoryResult = await client.query(
          `SELECT id, quantity FROM inventory
           ...
           FOR UPDATE`,
          [effectiveOutletId, pidNum, variantIdNorm],
        );

        if (inventoryResult.rows.length === 0) {
          await client.query(
            `INSERT INTO inventory (outlet_id, product_id, variant_id, quantity, minimum_stock)
             VALUES ($1, $2, $3, 0, 10) ON CONFLICT DO NOTHING`,
            [effectiveOutletId, pidNum, variantIdNorm],
          );
        ...
        await client.query(
          `UPDATE inventory SET quantity = $1, updated_at = CURRENT_TIMESTAMP
           WHERE outlet_id = $2 AND product_id = $3 AND variant_id IS NOT DISTINCT FROM $4`,
          [newQuantity, effectiveOutletId, pidNum, variantIdNorm],
        );

        await client.query(
          `INSERT INTO stock_ledger (outlet_id, product_id, variant_id, movement_type, quantity_change, notes, created_by)
      ...
      let inv = mockInventory.find(
        (i) =>
          i.outlet_id === outletIdNum &&
          i.product_id === pidNum &&
          (variantIdNorm !== null
      ...
      const prod = mockProducts.find((p) => p.id === pidNum);
      mockLedger.push({
        id: nextLedgerId(),
        outlet_id: outletIdNum,
        product_id: pidNum,
...
export const getStockLedger = async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    `;
    const params: any[] = [Number(effectiveOutletId)];
    ...
    if (isDbConnectionError(error)) {
      ...
      let rows = mockLedger.filter(
        (l) => l.outlet_id === Number(effectiveOutletId),
      );
      if (productId)
        rows = rows.filter((l) => l.product_id === Number(productId));
      if (startDate)
        rows = rows.filter(
          (l) => new Date(l.created_at) >= new Date(String(startDate)),
        );
      if (endDate)
        rows = rows.filter(
          (l) => new Date(l.created_at) <= new Date(String(endDate)),
        );
      rows.sort(
        (a, b) =>
...
export const checkLowStock = async (req: AuthRequest, res: Response) => {
  const effectiveOutletId = getEffectiveOutletId(req);
  if (!effectiveOutletId)
    return res.status(400).json({ error: "Outlet context required" });
  ...
  try {
    const result = await query(
      `SELECT i.id, i.product_id, i.quantity, i.minimum_stock, p.sku, p.name
       FROM inventory i
       JOIN products p ON i.product_id = p.id
       WHERE i.outlet_id = $1 AND i.quantity <= i.minimum_stock
       ORDER BY i.quantity ASC`,
      [Number(effectiveOutletId)],
    );

    res.json({
      lowStockItems: result.rows.map((row: any) => ({
        id: row.id,
        productId: row.product_id,
        sku: row.sku,
        name: row.name,
    ...
    if (isDbConnectionError(error)) {
      const rows = mockInventory.filter(
        (i) =>
      ...
      return res.json({
        lowStockItems: rows.map((r: any) => {
          const p = mockProducts.find((pp) => pp.id === r.product_id);
          return {
            id: r.id,
