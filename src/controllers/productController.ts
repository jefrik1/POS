...
export const createProduct = async (req: AuthRequest, res: Response) => {
  try {
    const { sku, name, description, categoryId, basePrice, costPrice } =
      req.body;
    ...
    const catCheck = await query("SELECT id FROM categories WHERE id = $1", [
      categoryIdNum,
    ]);
    if (catCheck.rows.length === 0) {
      return res.status(400).json({
        error:
          "Kategori tidak ditemukan. Jalankan: psql -d pos_db -f src/config/schema.sql",
      });
    }

    const result = await query(
      `INSERT INTO products (sku, name, description, category_id, base_price, cost_price)
       VALUES ($1, $2, $3, $4, $5, $6)
    ...
    if (isDbConnectionError(error)) {
      const { sku, name, description, categoryId, basePrice, costPrice } =
        req.body;
      if (mockProducts.some((p) => p.sku === sku)) {
        return res.status(409).json({ error: "SKU sudah ada" });
      }
      const categoryIdNum = Number(categoryId);
      const bp = Number(basePrice);
      ...
      ensureInventoryForProduct(newProd.id);
...
export const createProductVariant = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const bodyPid = (req.body as any).productId;
    ...
    const result = await query(
      `INSERT INTO product_variants (product_id, sku, name, variant_value, price_adjustment)
       VALUES ($1, $2, $3, $4, $5)
...
export const getProducts = async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (isDbConnectionError(error)) {
      ...
      const q = String((req.query as any).search || "").toLowerCase();
      if (q)
        filtered = filtered.filter(
          (p) =>
            p.name.toLowerCase().includes(q) || p.sku.toLowerCase().includes(q),
        );
      const cid = (req.query as any).categoryId
        ? Number((req.query as any).categoryId)
        : null;
      if (cid) filtered = filtered.filter((p) => p.category_id === cid);
      filtered.sort((a, b) => a.name.localeCompare(b.name));

      const lim = Number((req.query as any).limit || 50);
      const off = Number((req.query as any).offset || 0);
      const sliced = filtered.slice(off, off + lim);

      return res.json({
        products: sliced.map((p: any) => ({
          ...p,
          basePrice: p.base_price / 100,
          costPrice: p.cost_price / 100,
        })),
...
export const getProductVariants = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT id, product_id, sku, name, variant_value, price_adjustment, is_active
       FROM product_variants WHERE product_id = $1 AND is_active = true ORDER BY name`,
      [productId],
    );
    res.json(
      result.rows.map((v: any) => ({
        ...v,
